...
def predict_sequence(sequence, out_dir, weights_type='full', config_file=None, batch_size=1):
    ...
    # 3. Load model
    if weights_type == 'attention':
        weights_path = os.path.join(out_dir, "weights_attention.pmt")
        ...
        model.load_attention(weights_path)
        print(f"Loaded attention weights from {weights_path}")
    else:
        weights_path = os.path.join(out_dir, "weights_full.pmt")
        if not os.path.exists(weights_path):
            raise FileNotFoundError(f"Full weights not found at {weights_path}")
        model = SincFold(attention_only=False, device=device, verbose=verbose)
